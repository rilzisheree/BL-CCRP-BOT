    ...
    const renderPage = async (pg: number) => {
      ...
      if (total === 0) {
        return {
          ...
          rows: [] as ActionRowBuilder<ButtonBuilder>[],
        };
      }
      ...
      const unbanRow = new ActionRowBuilder<ButtonBuilder>();
      for (const ban of bans) {
        unbanRow.addComponents(
          new ButtonBuilder()
